...
>
> **UPDATE (2026-07-16, later same day):** The user applied the equivalent of migrations `0008`/`0009` to the hosted project directly, via their own separate Supabase connection (outside this coding session's tool access — hence it wasn't visible to the tooling that produced this SUMMARY). Confirmed via `list_migrations` on `ancfwsgyzoostoidqzqj`: they now appear as `20260716122549_clients_grants` and `20260716122557_profiles_pm_clients_grants` (timestamp-versioned, since applied through a different path than this repo's sequential filenames). As expected, this was a safe no-op given the platform-default grants noted above.
