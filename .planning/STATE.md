...
Last activity: 2026-08-05 - Completed quick task 260805-fuu: Unificar Criar card/Importar do chat + botão Enviar pro Kanban no Chat (Parte 2 de Melhorias na criação de conteúdo)
...
- **Minor, pre-existing:** a hard page reload (F5/`page.goto`, not in-app link navigation) on `/pm/chat` can redirect to `/pm/clients` even when a client is active in `localStorage` — `chat-panel.tsx`'s `useSyncExternalStore` snapshot reads `null` on the very first render, and the redirect-guard effect can fire before the real value syncs. Discovered live during 260805-fuu's verification, out of scope there (D-F), not yet fixed. Low impact — only literal browser refresh triggers it, never a click within the app.
